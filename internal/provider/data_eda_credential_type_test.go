...
func TestAccEdaCredentialTypeDataSource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccEdaCredentialTypeDataSourceConfig(resource1),
				ConfigStateChecks: []statecheck.StateCheck{
					statecheck.ExpectKnownValue(
						fmt.Sprintf("data.%s_eda_credential_type.test-name", configprefix.Prefix),
						tfjsonpath.New("name"),
					...
					statecheck.ExpectKnownValue(
						fmt.Sprintf("data.%s_eda_credential_type.test-id", configprefix.Prefix),
						tfjsonpath.New("name"),
						knownvalue.StringExact(resource1.Name),
					),
					statecheck.ExpectKnownValue(
...
func testAccEdaCredentialTypeDataSourceConfig(resource EdaCredentialTypeAPIModel) string {
	return fmt.Sprintf(`
...
data "%[1]s_eda_credential_type" "test-id" {
  id = %[1]s_eda_credential_type.test.id
}

data "%[1]s_eda_credential_type" "test-name" {
  name = %[1]s_eda_credential_type.test.name
}
